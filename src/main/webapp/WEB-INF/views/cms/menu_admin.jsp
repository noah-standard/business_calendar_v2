...
<h3 class="uk-heading-divider uk-padding-small">관리자메뉴관리</h3>
<div class="uk-flex uk-margin-medium-left uk-margin-medium-right">
    <div class="uk-panel uk-panel-scrollable uk-width-1-2 uk-padding" style="height:500px;">
        <p class="uk-text-right">
            ...
            <a href="/cms/config"><button class="uk-button uk-button-primary" type="button">메뉴등록</button></a>
        </p>
        <div id="jstree_div">
            <li>테스트</li>
        </div>
    </div>
    <form class="uk-width-1-2 uk-padding uk-form-horizontal" name="admin_menu_form" method="post">
...
<script>
    ...
    $(function () {
        document.querySelector("#edit").addEventListener("click",function(e){
            if(confirm('수정하시겠습니까?')){
                let form = document.forms.admin_menu_form;
                form.action = "/cms/ajax/admin_config/menu";
                form.submit();
            }
        });
                        ...
                        if (operation === "move_node") {
                            if(more.ref == undefined){
                                if(confirm('위치를 바꾸시겠습니까?')){
                                    ...
                                    xhr.open('POST', '/cms/admin_config/menu/edit');
        ...
        $("#jstree_div").bind('select_node.jstree', function(event, data){
            ...
            xhr.open('GET', '/cms/ajax/admin_config/menu?node='+id);
            xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8'); // 컨텐츠타입을 json으로
            xhr.send(); // 요청 전송
        })
        ...
        document.querySelector("#delete").addEventListener("click",function (){
            if(confirm('해당 메뉴를 삭제하시겠습니까?')){
                ...
                xhr.open('GET', '/cms/ajax/admin_config/menu/delete?idx='+idx);
                xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8'); // 컨텐츠타입을 json으로
                xhr.send(); // 요청 전송
            }
        });
